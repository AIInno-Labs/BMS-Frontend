-- Separate factory due date (manual) from Quotient quote valid_until (read-only on job card).
...
SET quote_valid_until = q.valid_until
FROM public.quotes q
WHERE j.quote_id = q.id
  AND q.valid_until IS NOT NULL
  AND j.quote_valid_until IS NULL;

UPDATE public.jobs
SET quote_valid_until = due_date
WHERE id LIKE 'JOB-Q-%'
